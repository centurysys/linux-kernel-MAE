...
static inline struct dsk324sr *to_dsk324sr_priv(struct rtc_device *rtcdev)
{
	return container_of(rtcdev, struct dsk324sr, rtc);
}
...
static int dsk324sr_get_datetime(struct i2c_client *client, struct rtc_time *tm)
{
	...
	err = i2c_smbus_read_i2c_block_data(client, DSK324SR_REG_SC, 7, date);
...
static int dsk324sr_rtc_read_time(struct rtc_device *rtc_dev, struct rtc_time *tm)
{
	struct dsk324sr *dsk324sr = to_dsk324sr_priv(rtc_dev);
	struct i2c_client *client = dsk324sr->client;

	return dsk324sr_get_datetime(client, tm);
}

static int dsk324sr_rtc_set_time(struct rtc_device *rtc_dev, struct rtc_time *tm)
{
	struct dsk324sr *dsk324sr = to_dsk324sr_priv(rtc_dev);
	struct i2c_client *client = dsk324sr->client;

	return dsk324sr_set_datetime(client, tm);
}
			  ...
			  const struct i2c_device_id *id)
{
	...
	struct dsk324sr *dsk324;
	int ret, data, err;

	dsk324 = xzalloc(sizeof(*dsk324));

	dsk324->client = client;

	dsk324->rtc.ops = &dsk324sr_rtc_ops;
	dsk324->rtc.dev = dev;

	ret = rtc_register(&dsk324->rtc);
	...
	if (ret == 0) {
		...
		if (data >= 0 &&
			...
			err = i2c_smbus_write_byte_data(client, DSK324SR_REG_SEL,
							data);
			if (err == 0) {
				dev_info(dev, "SEL Register updated to 30s.\n");
			} else {
				dev_info(dev, "SEL Register update failed.\n");
			}
		}
	}
