...
static struct irq_chip ksz_irq_chip = {
	.name			= "ksz-irq",
			    ...
			    const unsigned char *addr, u16 vid)
{
	...
	return dev->dev_ops->fdb_add(dev, port, addr, vid);
}
			    ...
			    const unsigned char *addr, u16 vid)
{
	...
	return dev->dev_ops->fdb_del(dev, port, addr, vid);
}
			    ...
			    const struct switchdev_obj_port_mdb *mdb)
{
	...
	return dev->dev_ops->mdb_add(dev, port, mdb);
}
			    ...
			    const struct switchdev_obj_port_mdb *mdb)
{
	...
	return dev->dev_ops->mdb_del(dev, port, mdb);
}
...
static void ksz_phylink_validate(struct dsa_switch *ds, int port,
				 unsigned long *supported,
				 struct phylink_link_state *state)
{
	struct ksz_device *dev = ds->priv;

	if (dev->dev_ops->phylink_validate)
		dev->dev_ops->phylink_validate(dev, port, supported, state);
}
...
static const struct dsa_switch_ops ksz_switch_ops = {
	...
	.phylink_validate	= ksz_phylink_validate,
