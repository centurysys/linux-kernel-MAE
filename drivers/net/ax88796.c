...
#ifndef CONFIG_MACH_MA8XX
	ret = request_irq(dev->irq, ax_ei_interrupt, 0, dev->name, dev);
#else
	ret = request_irq(dev->irq, ax_ei_interrupt, IRQF_TRIGGER_LOW,
                          dev->name, dev);
#endif
	if (ret)
		return ret;
...
static int ax_init_dev(struct net_device *dev, int first_init)
{
	...
	if (ret)
        ...
        {
                extern void ma8xx_get_ethaddr(unsigned char *dev_addr);

                ma8xx_get_ethaddr(dev->dev_addr);
        }
#endif
