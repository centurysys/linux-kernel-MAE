...
extern int magnolia2_get_fec_int(void);
static void mii_parse_phycr(uint mii_reg, struct net_device *dev)
{
	struct fec_enet_private *fep = netdev_priv(dev);
	volatile uint *s = &(fep->phy_status);
	uint status, mode;
...
/* register definitions for the 8041 */

#define MII_KS8041NL_RXERCR	0x15
#define MII_KS8041NL_ICSR	0x1B
#define	MII_KS8041NL_PHYCR	0x1F

static phy_cmd_t const phy_cmd_ks8041nl_config[] = {
		{ mk_mii_read(MII_REG_CR), mii_parse_cr },
		{ mk_mii_read(MII_REG_ANAR), mii_parse_anar },
		{ mk_mii_read(MII_KS8041NL_PHYCR), mii_parse_phycr },
		{ mk_mii_end, }
	};
static phy_cmd_t const phy_cmd_ks8041nl_startup[] = {  /* enable interrupts */
		{ mk_mii_write(MII_REG_CR, 0x1200), NULL }, /* autonegotiate */
		{ mk_mii_write(MII_KS8041NL_ICSR, 0x0d00), NULL },
		{ mk_mii_read(MII_REG_SR), mii_parse_sr },
		{ mk_mii_read(MII_KS8041NL_PHYCR), mii_parse_phycr },
		{ mk_mii_end, }
	};
static phy_cmd_t const phy_cmd_ks8041nl_ack_int[] = {
		/* find out the current status */
		{ mk_mii_read(MII_REG_SR), mii_parse_sr },
		{ mk_mii_read(MII_KS8041NL_PHYCR), mii_parse_phycr },
		/* we only need to read ISR to acknowledge */
		{ mk_mii_read(MII_KS8041NL_ICSR), NULL },
		{ mk_mii_end, }
	};
static phy_cmd_t const phy_cmd_ks8041nl_shutdown[] = { /* disable interrupts */
		{ mk_mii_write(MII_KS8041NL_ICSR, 0x0000), NULL },
		{ mk_mii_end, }
	};
static phy_info_t const phy_info_ks8041nl = {
	.id = 0x00022151,
	.name = "KS8041NL",
	.config = phy_cmd_ks8041nl_config,
	.startup = phy_cmd_ks8041nl_startup,
	.ack_int = phy_cmd_ks8041nl_ack_int,
	.shutdown = phy_cmd_ks8041nl_shutdown
};
#endif
...
static void __inline__ fec_request_intrs(struct net_device *dev)
{
	...
	if ((expio_intr_fec > 0) &&
            (request_irq(expio_intr_fec, mii_link_interrupt, IRQF_TRIGGER_LOW,
                         "fec(MII)", dev) != 0))
...
static int phy_int_enabled = 0;
static void __inline__ fec_enable_phy_intr(void)
{
        if (phy_int_enabled == 0 && expio_intr_fec > 0) {
...
static void __inline__ fec_disable_phy_intr(void)
{
        if (phy_int_enabled == 1 && expio_intr_fec > 0) {
		disable_irq(expio_intr_fec);
                phy_int_enabled = 0;
        }
}
...
phy_cmd_t const phy_cmd_relink[] = {
	{ mk_mii_read(MII_REG_SR), mii_queue_relink },
	{ mk_mii_end, }
	};
