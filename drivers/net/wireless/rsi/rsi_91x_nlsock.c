	...
	if (pkt_type == WLAN_PACKET) {
		...
		case UPDATE_WLAN_GAIN_TABLE:
			if (adapter->priv->fsm_state == FSM_MAC_INIT_DONE) {
				...
				rsi_update_wlan_gain_table(adapter, nlh,
							   payload_len);
			} else {
				rsi_dbg(INFO_ZONE,
					"%s: uninitialized fsm state\n",
		...
		case PER_TRANSMIT:
			if (adapter->priv->fsm_state == FSM_MAC_INIT_DONE) {
				...
				rsi_transmit_stats_cmd(adapter, nlh,
						       payload_len);
			} else {
				rsi_dbg(ERR_ZONE,
					"Driver not installed before issuing transmit command\n");
			}
			break;
		case PER_PACKET:
			...
			memcpy((&adapter->per_packet),
			       nlmsg_data(nlh) + FRAME_DESC_SZ, payload_len);
			break;
		case RSI_SET_BB_RF:
			adapter->wlan_nl_pid = pid;
			...
			memcpy((&adapter->bb_rf_params),
			       nlmsg_data(nlh) + FRAME_DESC_SZ, payload_len);
			if ((rsi_set_bb_rf_values(adapter)) < 0) {
				rsi_dbg(ERR_ZONE,
					"Invalid Arguments by user\n");
			}
			break;
		case RSI_EFUSE_MAP:
			adapter->wlan_nl_pid = pid;
		...
		case GET_RSSI:
			...
			if (adapter->device_model < RSI_DEV_9116) {
				rsi_dbg(ERR_ZONE,
					"GET_RSSI is not supproted in 9113\n");
				return;
			}
			current_time = jiffies_to_msecs(jiffies);
			if ((current_time - adapter->prev_rssi_fetch_time) <=
			    5000) {
				rsi_dbg(ERR_ZONE,
					"Time b/w get_rssi cmd is less than 5sec, updating previous RSSI\n");
		...
		case FILTER_BCAST:
			if (adapter->device_model < RSI_DEV_9116) {
				rsi_dbg(ERR_ZONE,
					"FILTER_BCAST feature is not supproted in 9113\n");
				return;
			}
			payload_len = nl_desc->desc_word[1];
			if (send_filter_broadcast_frame_to_fw
			    (adapter, nlh, payload_len) < 0) {
				rsi_dbg(ERR_ZONE,
					"Invalid Arguments by user\n");
			}
			break;
		...
		case BT_E2E_STAT:
			...
			payload_len = nl_desc->desc_word[1];
			rsi_hex_dump(DATA_TX_ZONE, "TX BT pkt", skb->data,
				     skb->len);
			status =
			    rsi_bt_e2e_stats(adapter, nlh, payload_len, cmd);
			if (status < 0)
		...
		case BT_BLE_GAIN_TABLE:
			...
			rsi_hex_dump(DATA_TX_ZONE, "TX BT pkt", skb->data,
				     skb->len);
			status =
			    rsi_bt_ble_update_gain_table(adapter, nlh,
							 payload_len, cmd);
			if (status < 0)
				rsi_dbg(ERR_ZONE,
					" Failed in BT/BLE GAIN TABLE UPDATE\n");
			break;
#endif
		default:
			rsi_dbg(ERR_ZONE, "Invalid Message Type\n");
