...
static void traffic_timer_callback(struct timer_list *t)
{
	...
	if (assoc) {
		if ((adapter->user_ps_en)
			...
			|| common->rx_data_inactive_interval)) {
			if (!adapter->wifi_paused
...
void check_data_load(struct rsi_hw *adapter)
{
	...
	if (adapter->ps_state == PS_ENABLED) {
		rsi_dbg(INFO_ZONE, "%s: Sending disable PS_req to TA in %d",
			__func__, __LINE__);
	...
	if (adapter->traffic_timer.function) {
		mod_timer(&adapter->traffic_timer,
			  msecs_to_jiffies(duration_i) + jiffies);
	} else {
...
void check_traffic_pwr_save(struct rsi_hw *adapter)
{
	if (adapter->user_ps_en
		...
		|| adapter->priv->disable_ps_from_lmac)) {
