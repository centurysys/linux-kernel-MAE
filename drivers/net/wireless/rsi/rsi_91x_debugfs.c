...
static int rsi_stats_read(struct seq_file *seq, void *data)
{
	...
	unsigned char fsm_state[][32] =
	    { "FSM_CARD_NOT_READY", "FSM_BOOT_PARAMS_SENT",
"FSM_EEPROM_READ_MAC_ADDR",
		"FSM_RESET_MAC_SENT", "FSM_RADIO_CAPS_SENT",
		    "FSM_BB_RF_PROG_SENT",
		"FSM_MAC_INIT_DONE"
	};
 ...
 */
static ssize_t rsi_debug_zone_write(struct file *filp, const char __user * buff,
...
static int rsi_bgscan_int_read(struct seq_file *file, void *data)
{
	...
	seq_printf(file,
		   ...
		   params->passive_scan_duration, params->two_probe);
	...
	if (common->debugfs_bgscan) {
		...
		for (cnt = 0; cnt < params->debugfs_bg_channels; cnt++) {
			if (params->debugfs_channels[cnt] & (DFS_CHANNEL))
				seq_printf(file, "%d[DFS] ",
					   (params->
					    debugfs_channels[cnt] & 0x7FFF));
			else
				seq_printf(file, "%d ",
					   params->debugfs_channels[cnt]);
		}
	} else {
		...
		for (cnt = 0; cnt < params->num_bg_channels; cnt++) {
			if (params->channels2scan[cnt] & (DFS_CHANNEL))
				seq_printf(file, "%d[DFS] ",
					   (params->
					    channels2scan[cnt] & 0x7FFF));
			else
				seq_printf(file, "%d ",
					   params->channels2scan[cnt]);
		}
	}
	seq_printf(file, "\n");
...
int rsi_validate_debugfs_bgscan_channels(struct rsi_common *common)
{
...
#endif
				valid_debugfs_channels[num_valid_channels] =
				    ch_num;
				num_valid_channels++;
				break;
			}
	...
	if (num_valid_channels > 0) {
		for (ii = 0; ii < num_valid_channels; ii++) {
			bgscan_info->debugfs_channels[ii] =
			    valid_debugfs_channels[ii];
			if ((common->bgscan_info.debugfs_channels[ii] >= 52)
			    && (common->bgscan_info.debugfs_channels[ii] <=
				140))
				common->bgscan_info.debugfs_channels[ii] |=
				    DFS_CHANNEL;
		}
...
static int rsi_validate_bgscan_params(struct rsi_common *common,
				      int *bgscan_vals, int user_params_cnt)
{
	struct bgscan_config_params *bgscan_info = &common->bgscan_info;
	int cnt = 0;
	...
	if (!bgscan_info->active_scan_duration
		...
		rsi_dbg(ERR_ZONE,
			"Active scan duration should be greater"
	...
	if (!bgscan_info->passive_scan_duration
		...
		rsi_dbg(ERR_ZONE,
			"Passive scan duration should be greater"
			" than 0 and less than 256\n");
		return -EINVAL;
	}
	bgscan_info->two_probe = bgscan_vals[5];
	if (!
	    ((bgscan_info->two_probe && (bgscan_info->two_probe == 1))
	...
	if ((user_params_cnt - 7) != bgscan_vals[6]) {
		rsi_dbg(ERR_ZONE,
			"channels count is more than" " Provided Channels\n");
		return -EINVAL;
	}
	bgscan_info->debugfs_bg_channels = bgscan_vals[6];
	if (bgscan_info->debugfs_bg_channels > MAX_BG_CHAN_FROM_USER) {
		rsi_dbg(ERR_ZONE,
			"User provided channels  are more"
			" than MAX_BG_CHAN_FROM_USER\n");
		return -EINVAL;
	}
	memset(bgscan_info->debugfs_channels, 0, (MAX_BG_CHAN_FROM_USER * 2));
				...
				loff_t * ppos)
{
	...
	total_bytes =
	    simple_write_to_buffer(bgscan_buf, count, ppos, user_buff, count);
	if (total_bytes < 1)
		return -EINVAL;
	...
	ret = sscanf(bgscan_buf, "%d%n", (int *)&bgscan_enable, &t_bytes);
	if (ret <= 0)
		return -EINVAL;
	...
	while (1) {
		ret =
		    sscanf(bgscan_buf + bytes_read, "%d%n", &bgscan_vals[cnt++],
			   &t_bytes);
		if (ret <= 0)
			break;
		bytes_read += t_bytes;
	...
	kfree(bgscan_buf);
	if (rsi_validate_bgscan_params(common, bgscan_vals, user_params_cnt)) {
...
#define FOPS(fopen)                                                               \
  {                                                                               \
    .owner = THIS_MODULE, .open = (fopen), .read = seq_read, .llseek = seq_lseek, \
  }

#define FOPS_RW(fopen, fwrite)                                                                       \
  {                                                                                                  \
    .owner = THIS_MODULE, .open = (fopen), .read = seq_read, .llseek = seq_lseek, .write = (fwrite), \
  }
					 ...
					 size_t count, loff_t * ppos)
{
	...
	total_bytes =
	    simple_write_to_buffer(in_buf, sizeof(in_buf) - 1, ppos, user_buff,
				   count);
	if (total_bytes < 1)
		return -EINVAL;
	in_buf[sizeof(in_buf) - 1] = '\0';
	ret = sscanf(in_buf + bytes_read,
					...
					size_t count, loff_t * ppos)
{
	...
	ret = sscanf(in_buf + bytes_read,
		     ...
		     &frame_load_vals[15],
		     &frame_load_vals[16],
		     &frame_load_vals[17], &frame_load_vals[18]);
					 ...
					 size_t count, loff_t * ppos)
{
	...
	ret = sscanf(in_buf + bytes_read,
		     ...
		     &beacon_load_vals[15],
		     &beacon_load_vals[16],
		     &beacon_load_vals[17], &beacon_load_vals[18], str);
...
static int rsi_int_read_beacon_params(struct seq_file *file, void *data)
{
	...
	seq_printf(file,
		   ...
		   params->bssid[2],
		   params->bssid[3],
		   params->bssid[4], params->bssid[5], params->str);
	seq_puts(file, "\n");
...
int rsi_validate_ps_params(struct rsi_common *common, int *ps_params_vals)
{
	...
	if (ps_info->rx_threshold
		...
		rsi_dbg(ERR_ZONE,
			"Making RX Hysterisis value to RX threshold "
	...
	if (ps_info->tx_threshold
		...
		rsi_dbg(ERR_ZONE,
			"Making TX Hysterisis value to TX threshold "
			"as hysterisis value should not be greater than "
			"tx threshold\n");
		ps_info->tx_hysterisis = ps_info->tx_threshold;
	}
	...
	if ((ps_info->monitor_interval
		...
		rsi_dbg(ERR_ZONE,
			"Supported Monitor Interval Range"
			"is 10 to 30000ms\n");
		rsi_dbg(ERR_ZONE,
			"Making monitor interval to default value"
	...
	if ((ps_info->num_dtims_per_sleep > 10)
		...
		rsi_dbg(ERR_ZONE,
			"Supported Range is 0 to 10 dtim intervals\n");
		return -EINVAL;
	}
	ps_info->deep_sleep_wakeup_period = ps_params_vals[10];
	if (ps_info->deep_sleep_wakeup_period > 65535) {
		rsi_dbg(ERR_ZONE,
			"Please enter a valid value for" "sleep_duration\n");
		rsi_dbg(ERR_ZONE, "Supported Range is 0 to 65535\n");
		return -EINVAL;
	}
	if ((ps_info->sleep_type == 2) && !ps_info->deep_sleep_wakeup_period) {
		ps_info->deep_sleep_wakeup_period = 10;
	}
	if ((!ps_info->listen_interval_duration)
	    && (!ps_info->num_bcns_per_lis_int)
	    && (!ps_info->dtim_interval_duration)
	    && (!ps_info->num_dtims_per_sleep)) {
		rsi_dbg(ERR_ZONE,
			"Listen_interval and num_bcns_per_lis_int"
	...
	if ((ps_info->uapsd_wakeup_period)
		...
		rsi_dbg(ERR_ZONE,
			"UAPSD wakeup period value should be in"
				   ...
				   loff_t * ppos)
{
	...
	if (!ps_param_buf)
		return -ENOMEM;
	total_bytes =
	    simple_write_to_buffer(ps_param_buf, count, ppos, user_buff, count);
	...
	while (1) {
		ret =
		    sscanf(ps_param_buf + bytes_read, "%d%n",
			   &ps_params_vals[cnt++], &t_bytes);
		if (ret <= 0)
			break;
		bytes_read += t_bytes;
	}
				   ...
				   loff_t * ppos)
{
	...
	if (common->bgscan_ssid_len > 32) {
		rsi_dbg(ERR_ZONE, "The ssid should be <= to 32 characters\n");
		return -EINVAL;
	}
	total_bytes =
	    simple_write_to_buffer(common->bgscan_ssid, count, ppos, user_buff,
				   count);
	if (total_bytes < 1)
		return -EINVAL;
...
static const struct rsi_dbg_files dev_debugfs_files[] = {
	  ...
	  },
	{
	 "debug_zone",
	 0666,
	 FOPS_RW(rsi_debug_read, rsi_debug_zone_write),
	  },
	{
	 "bgscan",
	 0666,
	 FOPS_RW(rsi_bgscan_read, rsi_bgscan_write),
	  },
	{
	 "ps_params",
	 0666,
	 FOPS_RW(rsi_read_ps_params, rsi_write_ps_params),
	  },
	{
	 "bgscan_ssid",
	 0666,
	 FOPS_RW(rsi_get_bgscan_ssid, rsi_set_bgscan_ssid),
	  },
#if defined(CONFIG_RSI_11K) && defined(RSI_DEBUG_RRM)
	{
	 "rrm_chan_load_req",
	 0666,
	 FOPS_RW(rsi_read_chload_meas_req, rsi_write_chload_meas_req),
	  },
	{
	 "rrm_frame_req",
	 0666,
	 FOPS_RW(rsi_read_frame_meas_req, rsi_write_frame_meas_req),
	  },
	{
	 "rrm_beacon_req",
	 0666,
	 FOPS_RW(rsi_read_beacon_meas_req, rsi_write_beacon_meas_req),
	  },
#endif
	{
	 "sdio_stats",
	 0644,
	 FOPS(rsi_sdio_stats_open),
	  },
};
...
int rsi_init_dbgfs(struct rsi_hw *adapter)
{
	...
	for (ii = 0; ii < adapter->num_debugfs_entries; ii++) {
		...
		dev_dbgfs->rsi_files[ii] =
		    debugfs_create_file(files->name, files->perms,
					dev_dbgfs->subdir, common,
					&files->fops);
	}
	return 0;
}
