...
enum per_commands { PER_RECEIVE = 2, PER_RECV_STOP = 6, PER_TRANSMIT =
	    1, PER_PACKET = 8 };
...
#else
enum edca_queue { BK_Q =
	    0, BE_Q, VI_Q, VO_Q, BK_Q_AP, BE_Q_AP, VI_Q_AP, VO_Q_AP,
	    MGMT_SOFT_Q, MGMT_BEACON_Q };
#endif
...
enum vap_modes { NO_VAP = 0, AP_ALONE = 1 << 0, STA_ALONE = 1 << 1, CONCURRENT =
	    AP_ALONE | STA_ALONE };
#endif
...
struct rsi_host_intf_ops {
	...
	int (*read_reg_multiple)(struct rsi_hw * adapter, u32 addr, u8 * data,
				 u16 count);
	int (*reg_read)(struct rsi_hw * adapter, u32 addr, u8 * data);
	int (*write_reg_multiple)(struct rsi_hw * adapter, u32 addr, u8 * data,
				  u16 count);
	int (*master_reg_read)(struct rsi_hw * adapter, u32 addr,
			       u32 * read_buf, u16 size);
	int (*master_reg_write)(struct rsi_hw * adapter, unsigned long addr,
