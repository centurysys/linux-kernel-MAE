...
int rsi_stats_frame(struct rsi_hw *adapter)
{
	...
	mgmt_frame->desc_word[5] =
	    cpu_to_le16(adapter->false_cca_rssi_threshold);
...
int set_per_configurations(struct rsi_hw *adapter)
{
	...
	if (rsi_load_bootup_params(common) == 0) {
		rsi_dbg(INFO_ZONE,
			"%s: BOOTUP Parameters loaded successfully\n",
			__FUNCTION__);
	} else {
	...
	if (DEV_MODEL_9116) {
		...
		if (status) {
			rsi_dbg(ERR_ZONE,
				"%s: Failed to Send wlan_9116 features parameters\n",
				__FUNCTION__);
			return status;
		}
	}
...
int rsi_send_rx_stats_cmd(struct rsi_hw *adapter, struct nlmsghdr *nlh)
{
	...
	adapter->per_params.per_ch_bw =
	    (adapter->per_params.rate_flags) & (0x07);
	channel->hw_value = adapter->priv->channel =
	    adapter->per_params.channel;
	rsi_dbg(INFO_ZONE, "%s: \nch_width = %d recv_channel = %d\n", __func__,
		adapter->per_params.per_ch_bw, adapter->per_params.channel);
	...
	if (adapter->per_params.channel == 0xFF) {
		...
		status =
		    rsi_wait_event(&common->chan_set_event, EVENT_WAIT_FOREVER);
		if (status < 0)
			return status;
...
int prepare_per_pkt(struct rsi_hw *adapter, struct sk_buff *skb)
{
	...
	if ((adapter->per_packet.enable && !adapter->per_packet.insert_seq)
	    || !adapter->per_packet.enable)
	...
	frame_desc[2] =
	    cpu_to_le32((adapter->per_params.
			 rate & 0x3ff) | (ch_bw << 12) | greenfield);
...
int do_continuous_send(struct rsi_hw *adapter)
{
	...
	if (adapter->per_params.aggr_enable == 1) {
		if (adapter->no_of_per_fragments != 1) {
			...
			length =
			    (adapter->per_params.pkt_length -
			     ((adapter->per_params.aggr_count -
			       1) * PER_AGGR_LIMIT_PER_PKT));
		}
		adapter->no_of_per_fragments--;
	} else
		length =
		    (adapter->per_params.pkt_length >
		     1536) ? 1536 : adapter->per_params.pkt_length;
	len = length + FRAME_DESC_SZ + extended_desc - 4 /* CRC */ ;
	if (adapter->rsi_host_intf == RSI_HOST_INTF_USB) {
		skb = dev_alloc_skb(len
				    + (RSI_USB_TX_HEAD_ROOM + FRAME_DESC_SZ +
				       64 /*64 bit dword align req */  +
				       26 /*extended desc sz is variable */ ));
		if (!skb) {
			return -1;
		}
	...
	if (adapter->rsi_host_intf == RSI_HOST_INTF_SDIO) {
		status =
		    adapter->host_intf_ops->reg_read(adapter,
		...
		status =
		    adapter->host_intf_ops->master_reg_read(adapter,
							    adapter->
	...
	if (skb->len > (FRAME_DESC_SZ + MIN_802_11_HDR_LEN + extended_desc)) {
		seq_num =
		    cpu_to_le16(*(unsigned char *)
				(&skb->
				 data[FRAME_DESC_SZ + MIN_802_11_HDR_LEN +
				      extended_desc - 2])) >> 4;
		seq_num = ((seq_num + 1) % 4096);
		skb->data[FRAME_DESC_SZ + MIN_802_11_HDR_LEN + extended_desc -
			  2] = (seq_num << 4) & 0xff;
		skb->data[FRAME_DESC_SZ + MIN_802_11_HDR_LEN + extended_desc -
			  1] = ((seq_num << 4) >> 8);
	}
	if (adapter->per_packet.enable)
		memcpy(&skb->data[FRAME_DESC_SZ + extended_desc],
			...
			adapter->per_params.pkt_length) ? (adapter->per_params.
							   pkt_length)
		       : (adapter->per_packet.length));
...
int send_per_frame(struct rsi_hw *adapter, unsigned char mode)
{
	...
	mgmt_frame->desc_word[0] =
	    cpu_to_le16(RSI_WIFI_MGMT_Q << 12 | sizeof(adapter->per_params));
	mgmt_frame->desc_word[1] = cpu_to_le16(PER_CMD_PKT);
	mgmt_frame->desc_word[3] = cpu_to_le16(mode);
	memcpy(&skb->data[FRAME_DESC_SZ], &adapter->per_params,
	       sizeof(adapter->per_params));
...
int send_per_ampdu_indication_frame(struct rsi_common *common)
{
	...
	mgmt_frame->desc_word[7] =
	    cpu_to_le16(0 | (START_AMPDU_AGGR << 4) | (0 << 8));
...
int start_per_tx(struct rsi_hw *adapter)
{
	...
	if (adapter->per_params.enable) {
		...
		if (adapter->per_params.channel <= 14
		    && (!adapter->per_params.enable_11j))
			set_band = NL80211_BAND_2GHZ;
		if ((adapter->per_params.channel >= 36)
		    && (adapter->per_params.channel <= 165))
...
SEND_PER_CMD:
		...
		if (adapter->per_params.mode == PER_CONT_MODE) {
			rsi_dbg(INFO_ZONE,
				"Enabling the PER continuous mode\n");
			if (do_continuous_send(adapter)) {
				rsi_dbg(ERR_ZONE,
					"Can't send do_continuous failed : PER\n");
				return -1;
			}
...
int rsi_transmit_stats_cmd(struct rsi_hw *adapter, struct nlmsghdr *nlh,
			   int payload)
{
	struct rsi_common *common = adapter->priv;
	unsigned short country_code;
	if (common->driver_mode == RF_EVAL_MODE_ON) {
		memcpy((&adapter->per_params), nlmsg_data(nlh) + FRAME_DESC_SZ,
		       payload);
		if (adapter->per_params.enable) {
			...
			if (!(common->band == NL80211_BAND_5GHZ)
			    && ((adapter->per_params.channel >= 36)
				|| (adapter->per_params.enable_11j))) {
				rsi_dbg(ERR_ZONE,
					"Invalid Channel Number for Module type\n");
				return -1;
			}
		}
