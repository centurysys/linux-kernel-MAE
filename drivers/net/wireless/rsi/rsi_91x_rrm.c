...
int rsi_rrm_send_channel_load_req(struct rsi_common *common)
{
	...
	chload_req_len =
	    sizeof(*chload_req) - sizeof(struct ieee80211_basic_hdr);
	chload_req_len -= sizeof(struct wl_action_req);
	chload_req_len -= sizeof(struct rm_element);
	chload_req->rmelem.length =
	    sizeof(struct rm_element) - 2 + chload_req_len;
...
int rsi_rrm_send_frame_req(struct rsi_common *common)
{
	...
	frame_req->rmelem.length =
	    sizeof(struct rm_element) - 2 + frame_req_len;
...
int rsi_rrm_send_beacon_req(struct rsi_common *common)
{
	...
	beacon_req->rmelem.length =
	    sizeof(struct rm_element) - 2 + beacon_req_len;
...
int rsi_rrm_parse_frame_req(struct rsi_common *common, struct sk_buff *skb,
...
int rsi_rrm_parse_beacon_req(struct rsi_common *common, struct sk_buff *skb,
			     struct rsi_beacon_meas_params *params)
{
	...
	while (index < skb->len) {
		...
		case SSID_ELEM_ID:
			...
			memcpy(&params->ssid_ie[2], &frm[index + 2],
			       opt_elem_len);
			rsi_dbg(ERR_ZONE, "SSID INFO %s %d\n",
				&params->ssid_ie[2], params->ssid_ie[1]);
			break;

		case BEACON_REPORT_INFO:
			flags = flags | (1 << 1);
			rsi_dbg(ERR_ZONE, "BEACON REPORT INFO\n");
			memcpy(params->bcn_rpt_info, &frm[index],
			       frm[index + 1] + 2);
			break;
...
int rsi_rrm_sched_req(struct rsi_common *common)
{
	...
	if (skb->data[1 + MIN_802_11_HDR_LEN] ==
		...
		case RRM_TYPE_CHANNEL_LOAD:
			...
			rsi_rrm_parse_channel_load_req(common, skb,
						       &chl_params);
			rsi_get_channel_load_meas(common, chl_params);
			memcpy((u8 *) & common->chload_meas,
			       (u8 *) & chl_params,
...
int rsi_rrm_parse_spectrum_action_frame(struct rsi_common *common,
					struct ieee80211_hdr *tmp_hdr, u8 * frm)
{
	return 0;
}
...
int rsi_prepare_channel_load_rpt(struct rsi_common *common, u8 * msg, int len)
{
	...
	chload_rpt->rmelem.length =
	    sizeof(struct rm_element) - 2 + chload_rpt_len;
...
int rsi_prepare_frame_rpt(struct rsi_common *common, u8 * msg, int len)
{
	...
	frame_rpt->rmelem.length =
	    sizeof(struct rm_element) - 2 + frame_rpt_len;
