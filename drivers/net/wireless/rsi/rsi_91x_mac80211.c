...
struct ieee80211_rate rsi_rates[12] = {
	{.bitrate = STD_RATE_01 * 5,.hw_value = RSI_RATE_1 }, {.bitrate =
							       STD_RATE_02 *
							       5,.hw_value =
							       RSI_RATE_2 },
	{.bitrate = STD_RATE_5_5 * 5,.hw_value = RSI_RATE_5_5 }, {.bitrate =
								  STD_RATE_11 *
								  5,.hw_value =
								  RSI_RATE_11 },
	{.bitrate = STD_RATE_06 * 5,.hw_value = RSI_RATE_6 }, {.bitrate =
							       STD_RATE_09 *
							       5,.hw_value =
							       RSI_RATE_9 },
	{.bitrate = STD_RATE_12 * 5,.hw_value = RSI_RATE_12 }, {.bitrate =
								STD_RATE_18 *
								5,.hw_value =
								RSI_RATE_18 },
	{.bitrate = STD_RATE_24 * 5,.hw_value = RSI_RATE_24 }, {.bitrate =
								STD_RATE_36 *
								5,.hw_value =
								RSI_RATE_36 },
	{.bitrate = STD_RATE_48 * 5,.hw_value = RSI_RATE_48 }, {.bitrate =
								STD_RATE_54 *
								5,.hw_value =
								RSI_RATE_54 },
};
...
static struct reg_map rsi_caracalla_reg_db[MAX_REG_COUNTRIES] = {
	{ "AU", NL80211_DFS_ETSI }, { "AT", NL80211_DFS_ETSI }, { "BE",
								 NL80211_DFS_ETSI
								 }, { "BR",
								     NL80211_DFS_WORLD
								     },
	{ "CA", NL80211_DFS_FCC }, { "CL", NL80211_DFS_WORLD }, { "CN",
								 NL80211_DFS_WORLD
								 }, { "CO",
								     NL80211_DFS_FCC
								     },
	{ "CZ", NL80211_DFS_ETSI }, { "DK", NL80211_DFS_ETSI }, { "FI",
								 NL80211_DFS_ETSI
								 }, { "FR",
								     NL80211_DFS_ETSI
								     },
	{ "DE", NL80211_DFS_ETSI }, { "HK", NL80211_DFS_WORLD }, { "IN",
								  NL80211_DFS_WORLD
								  }, { "ID",
								      NL80211_DFS_WORLD
								      },
	{ "IE", NL80211_DFS_ETSI }, { "IL", NL80211_DFS_ETSI }, { "IT",
								 NL80211_DFS_ETSI
								 }, { "JP",
								     NL80211_DFS_JP
								     },
	{ "KR", NL80211_DFS_WORLD }, { "LU", NL80211_DFS_ETSI }, { "MY",
								  NL80211_DFS_WORLD
								  }, { "MX",
								      NL80211_DFS_FCC
								      },
...
static int rsi_mac80211_get_chan_survey(struct ieee80211_hw *hw, int idx,
					struct survey_info *survey)
{
	...
	survey->filled =
	    SURVEY_INFO_NOISE_DBM | SURVEY_INFO_TIME | SURVEY_INFO_TIME_BUSY;
#else
	survey->filled =
...
struct ieee80211_vif *rsi_get_vif_using_vap_id(struct rsi_hw *adapter,
					       u8 vap_id)
{
	u8 i;
	...
	for (i = 0; i < RSI_MAX_VIFS; i++) {
		if (!adapter->vifs[i])
			continue;
		if (((struct vif_priv *)adapter->vifs[i]->drv_priv)->vap_id ==
		    vap_id)
		...
		if (common->enable_40mhz) {
			sbands->ht_cap.cap |=
			    (IEEE80211_HT_CAP_SUP_WIDTH_20_40 |
			     IEEE80211_HT_CAP_SGI_40 | ENABLE_RX_STBC);
		}
	}
...
#endif
{
	...
	if (vif->type == NL80211_IFTYPE_AP) {
		...
		rate_bitmap =
		    (common->fixedrate_mask[band] & sta->supp_rates[band]);
	}
		...
		if (vif->type == NL80211_IFTYPE_AP) {
			if (adapter->auto_chan_sel == ACS_DISABLE) {
				...
				rsi_dbg(MGMT_DEBUG_ZONE,
					"Auto Channel selection scan start\n");
			}
			adapter->idx = 0;
		}
		...
		if (!rsi_send_bgscan_params(common, 1)) {
			...
			if (scan_req->n_ssids > MAX_HW_SCAN_SSID) {
				...
				rsi_dbg(INFO_ZONE,
					...
					"hw scan ssid's\n",
					cfg_ssid->ssid, scan_req->n_ssids);
			}
 ...
 * Return: None.
 */
void rsi_indicate_tx_status(struct rsi_hw *adapter, struct sk_buff *skb,
			    int status)
{
			    ...
			    struct sk_buff *skb)
{
	...
	if ((vif->type == NL80211_IFTYPE_STATION) && (adapter->user_ps_en)
	    && (adapter->ps_info.monitor_interval)) {
					  ...
					  struct ieee80211_vif *vif)
{
	...
	for (i = 0; i < RSI_MAX_VIFS; i++) {
		if (adapter->vifs[i] == vif) {
			...
			case NL80211_IFTYPE_STATION:
				rsi_set_vap_capabilities(common, STA_OPMODE,
							 vif->addr,
							 vif_info->vap_id,
							 VAP_DELETE);
				break;
			...
			case NL80211_IFTYPE_P2P_CLIENT:
				...
				break;
			case NL80211_IFTYPE_P2P_GO:
				rsi_set_vap_capabilities(common, P2P_GO_OPMODE,
							 vif->addr,
							 vif_info->vap_id,
							 VAP_DELETE);
				break;
			...
			adapter->vifs[i] = NULL;
#ifdef CONFIG_STA_PLUS_AP
			clear_bit(vif_info->vap_id,
				  (void *)&adapter->vif_bit_map);
			common->vap_mode = NO_VAP;
			for (i = 0; i < RSI_MAX_VIFS; i++) {
				if (!adapter->vifs[i])
					continue;
				else if (adapter->vifs[i]->type ==
					 ...
					 bool newp2p)
{
	...
	rsi_set_vap_capabilities(common,
				 ...
				 vif->addr, vif_info->vap_id, VAP_DELETE);
	status =
	    rsi_set_vap_capabilities(common, intf_mode, vif->addr,
				     vif_info->vap_id, VAP_ADD);
#if 0
}

else
if ((intf_mode != common->last_vap_type) ||
(!ether_addr_equal(vif->addr, common->last_vap_addr))) {
status = rsi_set_vap_capabilities(common,
				  intf_mode,
				  vif->addr, vif_info->vap_id, VAP_UPDATE);
}
#endif
...
#endif
{
	...
	rsi_dbg(INFO_ZONE,
		...
		__func__, curchan->center_freq, curchan->flags, channel);
...
static int rsi_mac80211_config(struct ieee80211_hw *hw, u32 changed)
{
	...
	if (!vif) {
		rsi_dbg(ERR_ZONE,
			"vlm: %s[%d] We are running AP alone mode...\n",
			__func__, __LINE__);
	...
	if ((changed & IEEE80211_CONF_CHANGE_PS)
		...
		if ((conf->flags & IEEE80211_CONF_PS)
			...
			rsi_dbg(INFO_ZONE,
				"Device already Power Save Disabled State\n");
			adapter->user_ps_en = 0;
		}
					  ...
					  u32 changed)
{
	...
	if ((changed & BSS_CHANGED_BSSID)
		...
		if (common->band == NL80211_BAND_2GHZ)
			common->minimum_basic_rate =
			    rsi_rates[basic_rate].hw_value;

		else
			common->minimum_basic_rate =
			    rsi_rates[basic_rate + 4].hw_value;
	}
	if ((changed & BSS_CHANGED_ASSOC)
		...
		if (bss->assoc) {
			...
			rx_filter_word =
			    (ALLOW_DATA_ASSOC_PEER | ALLOW_CTRL_ASSOC_PEER |
			     ALLOW_MGMT_ASSOC_PEER | 0);
			if (DEV_MODEL_9116)
		...
		rsi_dbg(INFO_ZONE, "bssid=%02x:%02x:%02x:%02x:%02x:%02x",
		...
		rsi_dbg(INFO_ZONE,
			...
			bss->beacon_int,
			conf->listen_interval, bss->dtim_period);
				...
				const struct ieee80211_tx_queue_params *params)
{
	...
	rsi_dbg(INFO_ZONE,
		...
		queue,
		params->aifs,
		params->cw_min, params->cw_max, params->txop, params->uapsd);
	...
	memcpy(&common->edca_params[idx], params,
...
#ifdef CONFIG_STA_PLUS_AP
			status =
			    rsi_load_key(adapter->priv, key->key, key->keylen,
					 RSI_PAIRWISE_KEY, key->keyidx,
					 key->cipher, sta_id, vif);
#else
			status =
			    rsi_load_key(adapter->priv, key->key, key->keylen,
					 RSI_PAIRWISE_KEY, key->keyidx,
...
#ifdef CONFIG_STA_PLUS_AP
	status =
	    rsi_load_key(adapter->priv, key->key, key->keylen, key_type,
			 key->keyidx, key->cipher, sta_id, vif);
#else
	status =
	    rsi_load_key(adapter->priv, key->key, key->keylen, key_type,
			 key->keyidx, key->cipher, sta_id);
#endif
				...
				struct ieee80211_key_conf *key)
{
	...
	case SET_KEY:
		rsi_dbg(MGMT_DEBUG_ZONE,
			...
			(key->
			 flags & IEEE80211_KEY_FLAG_PAIRWISE) ? "PAIR_WISE" :
			"GROUP");
		...
		if (key->flags & IEEE80211_KEY_FLAG_PAIRWISE) {
			...
			case WLAN_CIPHER_SUITE_AES_CMAC:
				memcpy(vif_info->rx_bcmc_pn, seq.aes_cmac.pn,
				       6);
				vif_info->rx_pn_valid = true;
				vif_info->key = key;
				break;
...
#endif
{
	...
	case IEEE80211_AMPDU_RX_START:
		...
		status =
		    rsi_send_aggr_params_frame(common, tid, seq_no, buf_size,
					       STA_RX_ADDBA_DONE, sta_id);
		break;
	...
	case IEEE80211_AMPDU_RX_STOP:
		rsi_dbg(INFO_ZONE, "AMPDU action RX_STOP (%d) called\n",
			action);
		status =
		    rsi_send_aggr_params_frame(common, tid, 0, buf_size,
					       STA_RX_DELBA, sta_id);
		break;
...
#endif
		status =
		    rsi_send_aggr_params_frame(common, tid, seq_start, buf_size,
					       STA_TX_ADDBA_DONE, sta_id);
		break;
 ...
 */
static u16 rsi_get_rate_code(struct rsi_hw *adapter, u16 bitmap,
			     enum nl80211_band band, u8 rate_type)
{
	int i = 0;
				      ...
				      const struct cfg80211_bitrate_mask *mask)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
	enable_auto_rate = (mcs == MCS_RATE_MAP)
	...
	if (enable_auto_rate) {
		...
		if (mcs != MCS_RATE_MAP)
			rate =
			    rsi_get_rate_code(adapter, (u16) mcs, band,
					      MCS_RATE);
		else {
			for (i = 11; i >= 0; i--) {
				if (legacy & BIT(i)) {
			...
			rate =
			    rsi_get_rate_code(adapter, legacy, band,
					      LEGACY_RATE);
		}
...
static void rsi_perform_cqm(struct rsi_common *common, u8 * bssid, s8 rssi)
{
	...
	rsi_dbg(INFO_ZONE, "CQM: Notifying event: %s\n",
		(event ==
		 NL80211_CQM_RSSI_THRESHOLD_EVENT_LOW) ? "LOW" : "HIGH");

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 11, 0))
#ifdef CONFIG_STA_PLUS_AP
	ieee80211_cqm_rssi_notify(vif, event, rssi, GFP_KERNEL);
#else
	ieee80211_cqm_rssi_notify(adapter->vifs[adapter->sc_nvifs - 1], event,
				  rssi, GFP_KERNEL);
#endif
#else
#ifdef CONFIG_STA_PLUS_AP
				...
				struct ieee80211_sta *sta)
{
	...
	if ((vif->type == NL80211_IFTYPE_AP)
		...
		for (i = 0; i < common->max_stations; i++) {
			...
			if (!memcmp
			    (common->stations[i].sta->addr, sta->addr,
			     ETH_ALEN)) {
		...
		if (!sta_exist) {
			...
			if (common->key
				...
				rsi_load_key(adapter->priv, common->key->key,
					     common->key->keylen,
					     RSI_PAIRWISE_KEY,
					     common->key->keyidx,
					     common->key->cipher,
 ...
 * Return: 0 on success, -1 on failure.
 */
static int rsi_mac80211_set_antenna(struct ieee80211_hw *hw, u32 tx_ant,
				    u32 rx_ant)
{
	struct rsi_hw *adapter = hw->priv;
	struct rsi_common *common = adapter->priv;
 ...
 * Return: 0 on success, -1 on failure.
 */
static int rsi_mac80211_get_antenna(struct ieee80211_hw *hw, u32 * tx_ant,
				    u32 * rx_ant)
{
	struct rsi_hw *adapter = hw->priv;
	struct rsi_common *common = adapter->priv;
			   ...
			   struct regulatory_request *request)
{
	...
	rsi_dbg(INFO_ZONE,
		...
		__func__,
		request->alpha2, regdfs_region_str(request->dfs_region));

	region_code = request->dfs_region;
#ifdef CONFIG_CARACALLA_BOARD
	for (i = 0; i < ARRAY_SIZE(rsi_caracalla_reg_db); i++) {
		if (!memcmp
		    (rsi_caracalla_reg_db[i].country_code, request->alpha2,
		     2)) {
...
static const struct wiphy_wowlan_support rsi_wowlan_support = {
	.flags =
	    WIPHY_WOWLAN_ANY | WIPHY_WOWLAN_MAGIC_PKT | WIPHY_WOWLAN_DISCONNECT
	    | WIPHY_WOWLAN_GTK_REKEY_FAILURE | WIPHY_WOWLAN_SUPPORTS_GTK_REKEY |
...
void rsi_roc_timeout(struct timer_list *t)
{
	...
	if ((common->last_vap_type != intf_mode)
		...
		if (rsi_set_vap_capabilities
		    (common, intf_mode, vif->addr, vif_info->vap_id,
		     VAP_UPDATE))
			    ...
			    int duration, enum ieee80211_roc_type type)
{
	...
	if (vif->type == NL80211_IFTYPE_P2P_DEVICE) {
		...
		if (rsi_set_vap_capabilities
		    (common, P2P_CLIENT_OPMODE, vif->addr, vif_info->vap_id,
		     VAP_UPDATE)) {
			rsi_dbg(ERR_ZONE,
...
#ifdef CONFIG_STA_PLUS_AP
		rsi_send_sta_notify_frame(common, STA_OPMODE, STA_DISCONNECTED,
...
#else
	hw->flags =
	    IEEE80211_HW_SIGNAL_DBM | IEEE80211_HW_HAS_RATE_CONTROL |
	    IEEE80211_HW_AMPDU_AGGREGATION | IEEE80211_HW_SUPPORTS_PS |
	    IEEE80211_HW_SUPPORTS_DYNAMIC_PS | IEEE80211_HW_SPECTRUM_MGMT |
