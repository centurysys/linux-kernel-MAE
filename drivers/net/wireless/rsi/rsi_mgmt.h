...
enum opmode { UNKNOW_OPMODE = -1, AP_OPMODE = 0, STA_OPMODE = 1, P2P_GO_OPMODE =
	    2, P2P_CLIENT_OPMODE = 3 };

enum vap_status { VAP_ADD = 1, VAP_DELETE = 2, VAP_UPDATE = 3 };

enum peer_type { PEER_TYPE_AP, PEER_TYPE_STA, PEER_TYPE_P2P_GO,
	    PEER_TYPE_P2P_CLIENT, PEER_TYPE_IBSS };
...
int rsi_send_internal_mgmt_frame(struct rsi_common *common,
				 struct sk_buff *skb);
...
int rsi_send_sta_notify_frame(struct rsi_common *common,
			      ...
			      u8 qos_enable,
			      u16 aid, u16 sta_id, struct ieee80211_vif *vif);
int rsi_send_auto_rate_request(struct rsi_common *common,
			       struct ieee80211_sta *sta,
