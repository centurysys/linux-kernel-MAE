	...
	for (ii = 0; ii < NUM_EDCA_QUEUES; ii++) {
		...
		if (q_len) {
			if (tx_qinfo[ii].pkt_contended) {
				tx_qinfo[ii].weight =
				    ((tx_qinfo[ii].weight >
				      common->min_weight) ? tx_qinfo[ii].
				     weight - common->min_weight : 0);
			} else {
...
#else
		r_txop =
		    ieee80211_generic_frame_duration(adapter->hw,
...
void rsi_core_qos_processor(struct rsi_common *common)
{
	...
	while (1) {
		...
		status =
		    adapter->host_intf_ops->check_hw_queue_status(adapter,
								  q_num);
		if (status <= 0) {
...
struct rsi_sta *rsi_find_sta(struct rsi_common *common, u8 * mac_addr)
{
	...
	for (i = 1; i < common->max_stations; i++) {
		...
		if (!common->stations[i].sta)
			continue;
		if (!
		    (memcmp(common->stations[i].sta->addr, mac_addr, ETH_ALEN)))
...
void rsi_core_xmit(struct rsi_common *common, struct sk_buff *skb)
{
	...
	if (!(tx_params->flags & ENCAP_OFFLOAD_EN)
	    && ((ieee80211_is_mgmt(wlh->frame_control))
		...
		rsi_hex_dump(DATA_TX_ZONE, "TX Data Packet", skb->data,
			     skb->len);
		...
		if (!(tx_params->flags & ENCAP_OFFLOAD_EN)) {
			if ((vif->type == NL80211_IFTYPE_STATION)
				...
				if ((ieee80211_is_qos_nullfunc
				     (wlh->frame_control)
				     || ieee80211_is_nullfunc(wlh->
							      frame_control))) {
					++common->tx_stats.
					    total_tx_pkt_freed[skb->priority];
			...
			if (cpu_to_le16(*(u16 *) & skb->data[12]) ==
			    htons(ETH_P_IPV6)) {
				tos =
				    (u8) (ntohl
					  (cpu_to_le16
		...
		if (!(tx_params->flags & ENCAP_OFFLOAD_EN)) {
			...
			if ((!is_broadcast_ether_addr(ehdr->h_dest))
				...
				if (vif->type == NL80211_IFTYPE_AP
				    || vif->type == NL80211_IFTYPE_P2P_GO) {
					sta =
					    rsi_find_sta(common, ehdr->h_dest);
					if (!sta)
						goto xmit_fail;
					tx_params->sta_id = sta->sta_id;
				} else if (vif->type == NL80211_IFTYPE_STATION) {
      ...
      /**
			...
			if (!sta->start_tx_aggr[tid]
			    && !(info->control.
				 flags & IEEE80211_TX_CTRL_PORT_CTRL_PROTO)) {
	...
	if (q_num != MGMT_SOFT_Q) {
		...
		if ((skb_queue_len(&common->tx_queue[q_num]) + 1) >= water_mark) {
			...
			if (!ieee80211_queue_stopped
			    (adapter->hw, WME_AC(q_num)))
				ieee80211_stop_queue(adapter->hw,
						     WME_AC(q_num));
#else
			if (!ieee80211_queue_stopped
			    (adapter->hw, WME_AC(q_num % 4)))
				ieee80211_stop_queue(adapter->hw,
						     WME_AC(q_num % 4));
#endif
