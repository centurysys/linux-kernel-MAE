 ...
 *
 * @mors_if: pointer to driver's virtual interface
 * @ies_mask: array of pointers to information elements
 */
static void morse_insert_page_slice_element(struct morse_vif *mors_if,
					    struct dot11ah_ies_mask *ies_mask)
{
	struct ie_element *element;
	u32 element_size;
	struct page_slice_element *page_slice_elem;
	struct page_slicing *page_slicing_data = &mors_if->page_slicing_info;
	u32 page_bitmap = 0;
					    ...
					    u8 *page_index)
{
	...
	if (!dtim_count && virtual_map_len) {
		...
		morse_insert_page_slice_element(mors_if, ies_mask);
	}
...
void morse_page_slicing_init(struct ieee80211_vif *vif, u8 dtim_period, u8 enable)
{
	...
	else if (MORSE_CAPAB_SUPPORTED(&mors_if->capabilities, PAGE_SLICING))
