 ...
 * to be tested
 */
#define BUS_TEST_READ_WRITE_ADDRESS_LIST {0xDEADADD,}
...
int morse_bus_test(struct morse *mors, const char *bus_name)
{
	...
	for (size_idx = 0; size_idx < ARRAY_SIZE(cmp_size_list); size_idx++) {
		...
		for (address_idx = 0; address_idx < ARRAY_SIZE(address_list); address_idx++) {
			...
			ret = morse_bus_write_read_compare(mors, cmp_size, 0xAA, address_list[address_idx]);
			if (ret)
				goto exit;

			morse_info(mors, "%s: Clearing, Reading and verifying:\n", __func__);
			ret = morse_bus_write_read_compare(mors, cmp_size, 0, address_list[address_idx]);
			if (ret)
				goto exit;
		}
	}
