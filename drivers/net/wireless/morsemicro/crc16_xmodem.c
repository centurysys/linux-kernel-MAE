...
static const uint16_t table_byte[] = {
...
static const uint16_t table_word[][256] = {
...
uint16_t crc16xmodem_word(uint16_t crc, void const *mem, size_t len)
{
	...
	n = len >> 3;
	for (i = 0; i < n; i++) {
		word = crc ^ ((uint64_t const *)data)[i];
