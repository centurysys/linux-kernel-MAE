 ...
 */
static u8 *morse_dot11ah_wide_bw_chansw_ie(const u8 *ie, u8 *pos,
...
static u8 *morse_dot11ah_convert_ecsa_info_to_5g(u8 *frm_variable, u8 frm_variable_len, u8 *pos)
{
	...
	if (ecsa) {
		...
		/*
		 * Get HT channel for s1g_primary_channel from channel switch wrapper IE. For 2MHz
		 * pimary bandwidth get 1MHz primary channel(ht) at index 0. Considering channel
		 * only at 0'th index now.
		 */
		...
		chandef.chan->center_freq = ieee80211_channel_to_frequency(ecsa->new_ch_num,
									   NL80211_BAND_5GHZ);
		if (ie) {
			ie = cfg80211_find_ie(WLAN_EID_WIDE_BW_CHANNEL_SWITCH, (ie + 2), ie[1]);
			if (ie)
				pos = morse_dot11ah_wide_bw_chansw_ie(ie + 2, pos, ecsa, &chandef);
		}
