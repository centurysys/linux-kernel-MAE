 ...
 */
static int32_t morse_pv1_retrieve_tx_bpn(struct morse_vif *mors_if,
		...
		struct ieee80211_vif *vif, struct dot11ah_pv1_header_compression *hc_ie_data)
{
	...
	mutex_lock(&mors_if->pv1.lock);
	if (mors_if->pv1.tx_request.action_in_progress) {
		mors_if->pv1.tx_request.action_in_progress = false;
		mors_if->pv1.hc_response_timeout = jiffies;

		if (resp_status->a3_stored)
			memcpy(resp_status->stored_a3, mors_if->pv1.tx_request.stored_a3,
			       sizeof(resp_status->stored_a3));
		else
			memset(resp_status->stored_a3, 0, sizeof(resp_status->stored_a3));

		if (resp_status->a4_stored)
			memcpy(resp_status->stored_a4, mors_if->pv1.tx_request.stored_a4,
			       sizeof(resp_status->stored_a4));
		else
			memset(resp_status->stored_a4, 0, sizeof(resp_status->stored_a4));
	}

	mors_if->pv1.tx_pv1_sta = sta;

	if (resp_status->a3_stored || resp_status->a4_stored)
		schedule_work(&mors_if->pv1.hc_resp_work);
	mutex_unlock(&mors_if->pv1.lock);
}
					...
					struct morse *mors, struct ieee80211_vif *vif)
{
	...
	if (is_response)
		...
		morse_mac_process_hc_request(mors, mors_if, hc_ie_data, sta, dialog_token);
}
			...
			struct sk_buff *skb_data, bool is_response)
{
	...
	pv1 = &mors_if->pv1;
	tx = &mors_if->pv1.tx_request;
	rx = &mors_if->pv1.rx_request;
	sta_resp_status = &mors_sta->tx_pv1_ctx;
	timeout = jiffies - mors_if->pv1.hc_response_timeout;
 ...
 *
 * @mors_if:     Valid AP/STA VIF
 * @morse_sta:   A pointer to STA object
 * @hdr:         PV0 MAC Header
 */
static u16 morse_prepare_pv1_frame_ctrl(struct morse_vif *mors_if,
			struct morse_sta *mors_sta, struct ieee80211_hdr *hdr)
{
	struct morse_sta_pv1 *pv1_sta = &mors_sta->tx_pv1_ctx;
	struct morse_pv1_hc_request *tx_request = &mors_if->pv1.tx_request;
		...
		struct ieee80211_hdr *pv0_hdr, u16 fc)
{
	...
	if (vif->type == NL80211_IFTYPE_STATION)
		sid = cpu_to_le16(vif->bss_conf.aid & DOT11_MAC_PV1_SID_AID_MASK);
 ...
 * @mors_if: Morse vif structure
 * @sta:     Peer STA to which frame is being transmitted
 * @skb:     Tx data SKB
 *
 * @return: 0 on success, error on failure
 */
static int morse_convert_pv0_to_pv1(struct morse *mors, struct morse_vif *mors_if,
		struct ieee80211_sta *sta, struct sk_buff *skb)
{
	...
	pv1_fc = morse_prepare_pv1_frame_ctrl(mors_if, mors_sta, hdr);
	...
	if (is_protected) {
		...
		bpn = morse_pv1_retrieve_tx_bpn(mors_if, sta, seq_num, tid, is_mgmt);
		if (bpn < 0) {
			MORSE_ERR_RATELIMITED(mors,
 ...
 * @mors_if:     Valid AP iface
 * @aid:         Association ID of STA with AP
 *
 * @retun:       Pointer to STA context
 */
static struct ieee80211_sta *morse_pv1_find_sta_by_aid(struct morse_vif *mors_if, u16 aid)
{
				...
				struct dot11ah_mac_pv1_hdr *pv1_hdr)
{
	...
	if (pv1_fc_type == DOT11_MAC_PV1_FRAME_TYPE_QOS_DATA_SID) {
		...
		if (vif->type == NL80211_IFTYPE_AP)
			sta = morse_pv1_find_sta_by_aid(mors_if, aid);
 ...
 */
static int morse_prepare_pv0_mac_header(struct morse_vif *mors_if,
...
int morse_mac_convert_pv1_to_pv0(struct morse *mors, struct morse_vif *mors_if,
				...
				struct dot11ah_mac_pv1_hdr *pv1_hdr)
{
	...
	pv1_hdr_size = morse_prepare_pv0_mac_header(mors_if, &pv0_hdr, pv1_hdr, pv1_fc, pv0_fc);
...
static void morse_pv1_process_hc_resp_work(struct work_struct *work)
{
	...
	struct ieee80211_sta *sta = mors_if->pv1.tx_pv1_sta;
	...
	if (a3 || a4)
		morse_cmd_store_pv1_hc_data(mors, mors_if, sta, a3, a4, false);
}

int morse_mac_convert_pv0_to_pv1(struct morse *mors, struct morse_vif *mors_if,
		struct ieee80211_sta *sta, struct sk_buff *skb, int no_hwcrypt)
{
	...
	morse_pv1_a3_a4_check(mors_if, sta, skb);
	morse_mac_send_pv1_hc_action_frame(mors, vif, sta, no_hwcrypt, skb, 0);
	return morse_convert_pv0_to_pv1(mors, mors_if, sta, skb);
}

void mors_pv1_init_vif(struct morse_vif *mors_if)
{
	if (!mors_if || !mors_if->enable_pv1)
		return;

	mors_if->pv1.hc_response_timeout = 0;
	memset(&mors_if->pv1.tx_request, 0, sizeof(mors_if->pv1.tx_request));
	memset(&mors_if->pv1.rx_request, 0, sizeof(mors_if->pv1.rx_request));
	mutex_init(&mors_if->pv1.lock);
	INIT_WORK(&mors_if->pv1.hc_req_work, morse_pv1_process_hc_req_work);
	INIT_WORK(&mors_if->pv1.hc_resp_work, morse_pv1_process_hc_resp_work);
}

void morse_pv1_finish_vif(struct morse_vif *mors_if)
{
	if (!mors_if || !mors_if->enable_pv1)
		return;

	cancel_work_sync(&mors_if->pv1.hc_req_work);
	cancel_work_sync(&mors_if->pv1.hc_resp_work);
}
