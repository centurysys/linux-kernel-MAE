		    ...
		    const struct of_device_id *match_table)
{
	...
	dev_info(dev,
		"No pin configs found, using defaults...\n");
	cfg->mm_ps_async_gpio =
			MORSE_ASYNC_WAKEUP_FROM_CHIP_RPI_GPIO_DEFAULT;
	cfg->mm_wake_gpio =
			MORSE_WAKEPIN_RPI_GPIO_DEFAULT;
	cfg->mm_reset_gpio =
			MORSE_RESETPIN_RPI_GPIO_DEFAULT;
}
