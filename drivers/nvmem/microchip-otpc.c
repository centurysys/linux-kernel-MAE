				  ...
				  unsigned int offset)
{
	...
	ret = read_poll_timeout(readl_relaxed, tmp, !(tmp & MCHP_OTPC_SR_READ),
 ...
 * the range [offset1, offset1 + 0x4 + payload_size) the data registers are
 * updated by controller with packet 2. Header data is accessible though
 * MCHP_OTPC_HR register. Payload data is accessible though MCHP_OTPC_DR and
 * MCHP_OTPC_AR registers. There is no direct mapping b/w the offset requested
 * by software and the offset returned by hardware.
 *
 * For this, the read function will return the first requested bytes in the
 * packet. The user will have to be aware of the memory footprint before doing
 * the read request.
 */
static int mchp_otpc_read(void *priv, unsigned int pkt_id, void *val,
			  size_t bytes)
{
	...
	/*
	 * We reach this point with pkt_id being multiple of stride = 4 to
	 * be able to cross the subsystem. Inside the driver we use continuous
	 * unsigned integer numbers for packet id, thus devide pkt_id by 4
	 * before passing it to mchp_otpc_id_to_packet().
	 */
	pkt_id /= 4;
	packet = mchp_otpc_id_to_packet(otpc, pkt_id);
	if (!packet)
		return -EINVAL;
...
static int mchp_otpc_init_packet_list(struct mchp_otpc *otpc, u32 *size)
{
	struct mchp_otpc_packet *packet;
	u32 word, word_pos = 0, pos = 0, id = 0, npackets = 0, payload_size;
	int ret;

	INIT_LIST_HEAD(&otpc->packets);
	*size = 0;

	while (pos < MCHP_OTPC_SIZE) {
...
static int mchp_otpc_probe(struct platform_device *pdev)
{
	...
	otpc->dev = &pdev->dev;
	ret = mchp_otpc_init_packet_list(otpc, &size);
	if (ret)
		return ret;
