...
struct mchp_corespi {
	...
	const u8 *tx_buf;
	u8 *rx_buf;
	u32 clk_gen; /* output clock generated by the controller */
	u32 clk_mode;
	int irq;
	int tx_len;
	int rx_len;
...
static inline void mchp_corespi_clk_gen_set(struct mchp_corespi *spi)
{
	u32 control;
				     ...
				     struct spi_transfer *xfer)
{
	...
	spi_hz = min(xfer->speed_hz, master->max_speed_hz);

	if (spi_hz == 0 || spi_hz < master->min_speed_hz) {
		dev_err(&spi_dev->dev,
			"%s: %ld Hz too slow for system clock %ld Hz\n",
	 ...
	 * CLK_MODE = 1: SPICLK = 1 / (2 * CLK_GEN + 1) where CLK_GEN = 0 to 255.
	 * First try mode 1, fall back to 0 and f we have tried both modes and
	...
	if (clk_gen > CLK_GEN_MODE1_DIV_MAX || clk_gen <= CLK_GEN_DIV_MIN) {
		clk_gen = DIV_ROUND_UP(clk_hz, spi_hz);
		clk_gen = fls(clk_gen) - 1;

		if (clk_gen > CLK_GEN_MODE0_DIV_MAX || clk_gen < CLK_GEN_DIV_MIN) {
			dev_err(&spi_dev->dev,
				"%s: could not set clk_gen for spi clock %ld Hz & system clock %ld Hz\n",
				__func__, spi_hz, clk_hz);
			return -EINVAL;
		}
