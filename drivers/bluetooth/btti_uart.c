...
static void sm_process(struct btti_uart_dev *bdev, enum sm_event event)
{
	...
	case STATE_HW_ON:
		...
		case EVENT_HCI_WAKEUP_FRAME_RECEIVED:
			if (btti_uart_register_hci_device(bdev) == 0)
...
static inline struct llist_node *get_event_list(struct btti_uart_dev *bdev)
{
	struct llist_node *node;
					...
					const u8 *data, size_t count)
{
	...
	if (memcmp(&hw_wakeup_evt, data, sizeof(hw_wakeup_evt)) != 0)
		goto error;
...
MODULE_DESCRIPTION("TI CC33xx Bluetooth HCI UART driver");
MODULE_LICENSE("GPL");
