...
{
	int result, actlen;
        char buf[] = {'U',  'S',  'B',  'C',  0x00, 0x00, 0x00, 0x00,
...
/* This places the ZTE MF626/636 devices in multi-port mode */
int usb_stor_zte_init(struct us_data *us)
{
	int result, actlen;
        char buf[] = {0x55, 0x53, 0x42, 0x43, 0x12, 0x34, 0x56, 0x78,
...
/* This places the Softbank C01/02LC devices in multi-port mode */
int usb_stor_c01lc_init(struct us_data *us)
{
	int result, tmp, actlen;
        char buf[0x200] = {0x55, 0x53, 0x42, 0x43, 0x08, 0xc0, 0xaa, 0x85,
                           0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x06, 0x06,
                           0xf5, 0x04, 0x02, 0x52, 0x70, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

        printk("Softbank C01/02LC found, ejecting...\n");

        mdelay(500);
	result = usb_stor_bulk_transfer_buf(us, us->send_bulk_pipe,
                                            buf, 31, &actlen);
	tmp = usb_stor_bulk_transfer_buf(us, us->send_bulk_pipe,
                                         buf, 0x80, &actlen);
	tmp = usb_stor_bulk_transfer_buf(us, us->send_bulk_pipe,
                                         buf, 0x200, &actlen);
	US_DEBUGP("usb_bulk_msg performing result is %d\n", result);
