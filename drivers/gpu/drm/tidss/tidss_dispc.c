					 ...
					 const struct drm_display_mode *mode)
{
	...
	if (dispc->tidss->shared_mode && !dispc->tidss->shared_mode_owned_vps[hw_videoport]) {
		int vp_hdisplay = VP_REG_GET(dispc, hw_videoport, DISPC_VP_SIZE_SCREEN, GENMASK(11, 0)) + 1;
		int vp_vdisplay = VP_REG_GET(dispc, hw_videoport, DISPC_VP_SIZE_SCREEN, GENMASK(27, 16)) + 1;
...
{
	int r = 0, i = 0;
	const char *plane_names[TIDSS_MAX_PLANES];
	...
	shared_mode_feat->num_vids = device_property_string_array_count(dispc->dev,
	...
	/* Clear vid_info names */
	for (i = 0; i < TIDSS_MAX_PLANES; i++)
		shared_mode_feat->vid_info[i].name = NULL;

	/* Read plane names into temporary array */
	memset(plane_names, 0, sizeof(plane_names));
	r = device_property_read_string_array(dispc->dev, "ti,dss-shared-mode-planes",
					      plane_names, TIDSS_MAX_PLANES);
	if (r < 0) {
		dev_err(dispc->dev, "failed to read client vid layer name: %d\n", r);
		return r;
	}

	/* Copy plane names to vid_info and set is_lite based on name */
	for (i = 0; i < shared_mode_feat->num_vids; i++) {
		shared_mode_feat->vid_info[i].name = plane_names[i];
		if (!strncmp("vidl", plane_names[i], 4))
			shared_mode_feat->vid_info[i].is_lite = true;
		else
			shared_mode_feat->vid_info[i].is_lite = false;
	}
	...
	for (i = 0; i < shared_mode_feat->num_vids; i++) {
		dev_dbg(dispc->dev, "vid[%d] = %s, vid_order[%d] = %u vid_lite[%d] = %u\n", i,
			shared_mode_feat->vid_info[i].name, i, shared_mode_feat->vid_order[i], i,
			shared_mode_feat->vid_info[i].is_lite);
	}
...
{
	return REG_GET(dispc, DSS_SYSSTATUS, (1 << 9));
}
