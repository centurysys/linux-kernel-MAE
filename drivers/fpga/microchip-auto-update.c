...
static int mpfs_auto_update_write(struct fpga_manager *mgr, const char *buf, size_t count)
{
	...
	image_address = AUTO_UPDATE_BITSTREAM_BASE +
		AUTO_UPDATE_UPGRADE_INDEX * size_per_bitstream;
	...
	memcpy(buffer + AUTO_UPDATE_UPGRADE_DIRECTORY,
	       &image_address, AUTO_UPDATE_DIRECTORY_WIDTH);

	/* zero out the next directory so that the system controller doesn't complain */
	memset(buffer + AUTO_UPDATE_BLANK_DIRECTORY,
...
// This commented section needs to be dropped before application as it is
// replaced by the tamper macro.
//
//	/*
...
static int mpfs_auto_update_run(struct device *dev)
{
	...
	dev_info(dev, "starting to test the fpga manager\n");
	...
	if (ret) {
		dev_err(dev, "failed to lock the FPGA manager\n");
		goto free_info;
	}
	...
	dev_info(dev, "bitstream write complete\n");
...
static int mpfs_auto_update_probe(struct platform_device *pdev)
{
	...
	mgr = devm_fpga_mgr_create(dev, "Microchip MPFS Auto Update FPGA Manager",
				   &mpfs_auto_update_ops, priv);
	if (!mgr)
		return dev_err_probe(dev, PTR_ERR(mgr), "Could not create FPGA manager.\n");

	ret = devm_fpga_mgr_register(dev, mgr);
	if (ret)
		return dev_err_probe(dev, ret, "Could not register FPGA manager.\n");
