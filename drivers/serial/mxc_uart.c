...
#if defined(CONFIG_MACH_MAGNOLIA2) && defined(CONFIG_MXC_UART_DSR_GPIO)
                if (umxc->pin_DSR) {
                        int stat = mxc_get_gpio_datain(umxc->pin_DSR);
	...
	writel(cr1, umxc->port.membase + MXC_UARTUCR1);
        ...
        if (umxc->driver_type == 1) {
                if (umxc->driver_duplex == 0) {
                        ...
                        mxc_set_gpio_dataout(umxc->TxEnable, 0);
                        mxc_set_gpio_dataout(umxc->RxEnable, 0);
                }
        }
#endif
...
static void mxcuart_shutdown(struct uart_port *port)
{
	...
	if (umxc->dma_enabled == 1) {
        ...
        if (umxc->driver_type == 1) {
                /* Disable TxD/RxD */
                mxc_set_gpio_dataout(umxc->TxEnable, 0);
                mxc_set_gpio_dataout(umxc->RxEnable, 1);
        }
#endif
}
...
static const char *mxcuart_type(struct uart_port *port)
{
	...
	uart_mxc_port *umxc = (uart_mxc_port *) port;
        ...
        if (port->type == PORT_MXC) {
                if (umxc->driver_type == 0)
                        return "Freescale MXC (RS-232)";
                else {
                        if (umxc->driver_duplex == 0)
                                return "Freescale MXC (RS-485)";
                        else
                                return "Freescale MXC (RS-422)";
                }

                return "Freescale MXC";
        } else
                return NULL;
#else
