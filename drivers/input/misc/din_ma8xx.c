...
#define DIN_ENTRY_NAME "driver/din"
...
struct ma8xx_din_info {
	struct input_dev *idev;
	struct ma8xx_gpio_port *port;
};

extern void gpio_din_active(void);
extern void gpio_din_inactive(void);

static struct platform_device *ma8xx_pdev;

static inline u8 ma8xx_din_get(void)
{
	struct ma8xx_gpio_private *priv = ma8xx_pdev->dev.platform_data;
	struct ma8xx_din_info *info = platform_get_drvdata(ma8xx_pdev);
	int i;
        u8 val = 0;
...
static int ma8xx_din_read_proc(char *page, char **start, off_t off,
...
static int ma8xx_din_open(struct input_dev *idev)
{
	struct ma8xx_din_info *info = dev_get_drvdata(&idev->dev);
...
static void ma8xx_din_close(struct input_dev *idev)
{
	struct ma8xx_din_info *info = dev_get_drvdata(&idev->dev);
...
static irqreturn_t ma8xx_din_irq_handler(int irq, void *dev_id)
{
...
static int ma8xx_din_probe(struct platform_device *pdev)
{
	struct ma8xx_gpio_private *priv = pdev->dev.platform_data;
	struct ma8xx_din_info *info;
	int ret, i;
        unsigned long flags;

	info = kzalloc(sizeof(struct ma8xx_din_info) * priv->nr_gpio,
		       GFP_KERNEL);
	if (!info)
		return -ENOMEM;

	if (!create_proc_read_entry(DIN_ENTRY_NAME, 0, 0, ma8xx_din_read_proc, NULL)) {
		printk(KERN_ERR "%s: DIN create proc error\n", __FUNCTION__);
		return -1;
	}
	...
	for (i = 0; i < priv->nr_gpio; i++) {
		...
		info[i].idev->open = ma8xx_din_open;
		info[i].idev->close = ma8xx_din_close;
...
static int ma8xx_din_remove(struct platform_device *pdev)
{
	struct ma8xx_gpio_private *priv = pdev->dev.platform_data;
	struct ma8xx_din_info *info = platform_get_drvdata(pdev);
	int i;

	remove_proc_entry(DIN_ENTRY_NAME, NULL);
...
static struct platform_driver ma8xx_din_driver = {
	.probe	= ma8xx_din_probe,
	.remove = __devexit_p(ma8xx_din_remove),
	.driver	= {
		.name = "ma8xx_din",
	},
};
...
ma8xx_din_init(void)
{
	return platform_driver_register(&ma8xx_din_driver);
}

static void __exit
ma8xx_din_exit(void)
{
	platform_driver_unregister(&ma8xx_din_driver);
}

module_init(ma8xx_din_init);
module_exit(ma8xx_din_exit);

MODULE_AUTHOR("Century Systems, Co.,Ltd.");
MODULE_DESCRIPTION("MA-8xx Contact-IN driver");
