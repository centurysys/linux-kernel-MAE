 ...
 * Copyright 2014,2023 Century Systems.
 *
				    ...
				    const unsigned long *active_scan_mask)
{
	...
	for (i = 0; i < LTC185X_MAX_CHAN; i++) {
		if (test_bit(i, active_scan_mask)) {
			...
			st->ring_xfer[i].delay.value = 16;
			st->ring_xfer[i].delay.unit = SPI_DELAY_UNIT_USECS;
...
static const struct iio_chan_spec_ext_info ltc185x_ext_info[] = {
	...
	IIO_ENUM_AVAILABLE("ranges", IIO_SHARED_BY_TYPE, &ltc185x_range_enum),
	{},
};
...
static int ltc185x_probe(struct spi_device *spi)
{
	...
	st->scan_single_xfer[0].delay.value = 16;
	st->scan_single_xfer[0].delay.unit = SPI_DELAY_UNIT_USECS;
...
static void ltc185x_remove(struct spi_device *spi)
{
