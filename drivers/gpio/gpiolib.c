 ...
 *   /filter ... matching gpio_chip.filter (Magnolia2)
 */
...
static DEVICE_ATTR(filter, 0644, chip_filter_show, chip_filter_store);
#endif
...
#if defined(CONFIG_MACH_MAGNOLIA2) && defined(CONFIG_GPIO_MAGNOLIA2)
int gpio_counter_ctrl(unsigned gpio, int enable)
{
	unsigned long		flags;
	struct gpio_chip	*chip;
	struct gpio_desc	*desc = &gpio_desc[gpio];
...
EXPORT_SYMBOL_GPL(gpio_counter_ctrl);

int gpio_polarity_ctrl(unsigned gpio, int polarity)
{
	unsigned long		flags;
	struct gpio_chip	*chip;
	struct gpio_desc	*desc = &gpio_desc[gpio];
...
EXPORT_SYMBOL_GPL(gpio_polarity_ctrl);
#endif
